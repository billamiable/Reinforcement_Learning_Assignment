...
class Agent(object):
    ...
    def build_computation_graph(self):
        ...
        # print(self.sy_logprob_n.get_shape())
        # print(self.sy_adv_n.get_shape())
        # exit()
        # Define the actor, the same as before
        actor_loss = tf.reduce_sum(-self.sy_logprob_n * self.sy_adv_n)
        # actor_loss = tf.negative(tf.reduce_sum(tf.multiply(self.sy_logprob_n, self.sy_adv_n)))
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            # YOUR CODE HERE
            # raise NotImplementedError
            # print('second', np.shape(ob))
            # exit()
            next_obs.append(ob)
            rewards.append(rew)
            steps += 1
            # TO-DO: what is the meaning here?
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # Therefore, really to call twice
        ...
        vs_prime_tmp = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
        vs_prime = (1 - terminal_n) * vs_prime_tmp       
            
        adv_n = re_n + self.gamma * vs_prime - vs
        # print(np.shape(vs_prime_tmp))
        # print(np.shape(vs_prime))
        # print(np.shape(vs))
        # print(np.shape(adv_n))
        # print(terminal_n)
        # exit()
        if self.normalize_advantages:
            # raise NotImplementedError
            adv_n = norm(adv_n, 0.0, 1.0) # YOUR_HW2 CODE_HERE
        # Difference: here no q_n
        return adv_n
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        # raise NotImplementedError
        # Therefore, more target update is better, in each target, gradient step is not that important
        for i in range(self.num_target_updates):
            # here i need to define the input for training, self.sy_target_n
            # test if correct!
            vs_prime_tmp = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
            # print(np.shape(vs_prime_tmp))
            # print(np.shape(terminal_n))
            # exit()
            # print(vs_prime_tmp)
            vs_prime = (1 - terminal_n) * vs_prime_tmp
            
            # print(vs_prime)
            # print('i am here', np.shape(vs_prime))
            # exit()
            target_value = re_n + self.gamma * vs_prime
            # print('i am here', np.shape(target_value))
            for j in range(self.num_grad_steps_per_target_update):
                _, crit_loss = self.sess.run([self.critic_update_op, self.critic_loss], feed_dict={self.sy_target_n: target_value, self.sy_ob_no: ob_no})
