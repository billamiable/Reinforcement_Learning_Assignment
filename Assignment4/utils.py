...
class Dataset(object):
    ...
    # Difference between the next two function is fixed or random iteration
    ...
    def random_iterator(self, batch_size):
        ...
        i = 0
        # print('dataset size is', len(all_indices))
        while i < len(all_indices):
            indices = all_indices[i:i+batch_size]
            # Like return, but DO NOT STORE IN MEMORY
            yield states[indices], actions[indices], next_states[indices], rewards[indices], dones[indices]
    ...
    # It seems that i will first store them all into the dataset
    def log(self):
