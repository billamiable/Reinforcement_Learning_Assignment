...
def build_rnn(x, h, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None, regularizer=None):
    ...
    #====================================================================================#
    # YOUR CODE HERE
    # First using mlp to extract feature
    # TO-DO: figure out how to give input??
    # x = tf.reshape(x, (-1, x.get_shape()[1]*x.get_shape()[2]))
    # shape (?, 60, 6)
    # print('first size', x.get_shape())
    # VIP: here using n_layers is enough
    x = build_mlp(x, size, scope, n_layers, size, activation=activation, output_activation=activation)
    # shape (?, 60, 64)
    # print('mlp output', x.get_shape())
    # exit()
    # Then using GRU to do recurrent stuff
    # Figure out the input dim for GRU
    cell = tf.nn.rnn_cell.GRUCell(output_size, activation=activation)
    # shape (?, 60, 32) and (?, 32)
    outputs, last_states = tf.nn.dynamic_rnn(cell=cell, inputs=x, initial_state=h)
    # print('rnn output', outputs.get_shape())
    # print('rnn hidden state',last_states.get_shape())
    # exit()
...
def build_policy(x, h, output_size, scope, n_layers, size, gru_size, recurrent=True, activation=tf.tanh, output_activation=None):
    ...
    arguments:
        # TO-DO: still not get it for batch_size??
        ...
        # What do you mean by not counting recurrent units???
        ...
        # why still using gru_size for mlp?
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        if recurrent:
            print('using recurent')
            # exit()
            x, h = build_rnn(x, h, gru_size, scope, n_layers, size, activation=activation, output_activation=activation)

        else:
            print('not using recurent')
            # exit()
            # seems like flatten operation, since there are history and meta_ob
            # VIP: why need flatten? for mlp flatten is needed to operate fully-connect fashion
            # print('first',x.get_shape())
            x = tf.reshape(x, (-1, x.get_shape()[1]*x.get_shape()[2]))
            # print('reshape',x.get_shape())
            # print('gru size', gru_size)
            # print('output size', output_size)
            # VIP: Why add 1 for n_layers?? To comply with build_rnn
            # Attention: output_activation here is using activation
            # gru_size = 32
...
class Agent(object):
    def __init__(self, computation_graph_args, sample_trajectory_args, estimate_return_args):
        ...
        print('observation dim is',self.ob_dim)
        print('action dim is',self.ac_dim)
        print('meta_ob dim is',self.meta_ob_dim)
        # exit()
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        sy_lp_n = tf.contrib.distributions.MultivariateNormalDiag(
            loc=sy_mean, scale_diag=tf.exp(sy_logstd)).log_prob(sy_ac_na)
        return sy_lp_n
    ...
    def sample_trajectory(self, env, animate_this_episode, is_evaluation):
        ...
        # what is the self.history??? it is 1 at first.
        # I think it is the most up-to-date meta observation
        # self.meta_ob_dim is the concatenate of four input

        construct meta-observations by concatenating (s, a, r, d) into one vector
        # why have batch_size? only 1?
        inputs to the policy should have the shape (batch_size, self.history, self.meta_ob_dim)
        
        # what do you mean by zero pad?
        # I think it is needed to maintain the fixed-length input
        ...
        # why add 1 here? 
        # It is reasonable since it adds to max_path_length
        num_samples = max(self.history, self.max_path_length + 1)
       
        # Don't be too serious about the dim, the first and last are not used in this implementation
        meta_obs = np.zeros((num_samples + self.history + 1, self.meta_ob_dim))
        if 0:
            print('meta observation size is',np.shape(meta_obs))
            exit()
        rewards = []
        ...
        while True:
            ...
            if ep_steps == 0:
                ...
                if 0:
                    print('first observation is',ob)
                    print(self.ob_dim)
                    print('first insert place is',steps + self.history)
                    print(meta_obs)
                    print('output',meta_obs[(steps + self.history)])
                    exit()
                # why add second time?
                # Perhaps it is for convinent indexing
                steps += 1

            # what do you mean by window???
            # I think it has sth to do with history
            ...
            # YOUR CODE HERE
            # here needs to use ob_dim or meta_dim to conform with replay buffer
            ...
            # YOUR CODE HERE
            # TO-DO: check if self.sy_ob_no:[in_] is correct
            # Shape is (1,2)
            ...
            # YOUR CODE HERE
            # TO-DO: what should be constructed here?
            meta_ob = np.concatenate([ob, ac[0], [rew], [done]], axis=0)
            meta_obs[(steps + self.history)] = meta_ob
            if 0:
                print('here',meta_ob)
                print(np.shape(meta_ob))
                print(meta_obs)
                exit()
            rewards.append(rew)
            steps += 1
            ...
            # I believe it is iterate 4 goals
            if 0:
            ...
            if done:
                ...
                # It indicates for each trajectory, the episode length is ep_steps
                s = dict()
